...
firstHalf s = if (even (length s))
  then take ((length s) `div` 2) s
  else take (((length s) `div` 2) + 1) s

palindrome :: String -> Bool
palindrome s = if (s == (reverse s))
  then True
  else False
...
capitalize s = unwords (map (firstLetterCap) (words s))

firstLetterCap :: String -> String
firstLetterCap s = (charToString(toUpper(head s))) ++ (tail s)
...
whileRight f x = whileRightHelp f (f x)

whileRightHelp :: (a -> Either b a) -> Either b a -> b
whileRightHelp f (Right x) = whileRightHelp f (f x)
whileRightHelp f (Left x) = x
